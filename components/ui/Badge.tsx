...
export const BADGE_DEFS: Record<BadgeVariant, BadgeDef> = {
  ...
  // ── Pricing type (neutral slate — just a label, risk badge conveys severity) ──
  'spot': {
    label:   'Spot',
    classes: 'bg-slate-100 text-slate-600 dark:bg-white/5 dark:text-slate-400 border border-slate-200/60 dark:border-white/10',
  ...
  // ── Price highlight (emerald — positive) ──────────────────────────────────────
  'best': {
    label:   'BEST',
    classes: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-500/20 uppercase tracking-widest',
    tooltip: 'Cheapest option matching your current filters and sort.',
  },

  // ── Provider billing quirks (amber — caution) ─────────────────────────────────
  'idle-billing': {
    label:   'Idle Billing',
    classes: 'bg-amber-50 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400 border border-amber-200/60 dark:border-amber-500/20',
  ...
  'waitlist': {
    label:   'Waitlist',
    classes: 'bg-red-50 text-red-700 dark:bg-red-500/10 dark:text-red-400 border border-red-200/60 dark:border-red-500/20',
  ...
  'stable': {
    label:   'Stable',
    classes: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-500/20',
  ...
  'out-of-stock': {
    label:   'Out of Stock',
    classes: 'bg-slate-100 text-slate-500 dark:bg-white/5 dark:text-slate-400 border border-slate-200/60 dark:border-white/10',
    tooltip: 'Currently unavailable — no instances in this configuration can be allocated right now.',
  },

  // ── Value + Trends (emerald — positive) ───────────────────────────────────────
  'top-value': {
    label:   'TOP VALUE',
    classes: 'bg-emerald-50 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-200/60 dark:border-emerald-500/20 uppercase tracking-widest',
  ...
  'enterprise': {
    label:   'Enterprise',
    classes: 'bg-blue-50 text-blue-700 dark:bg-blue-500/10 dark:text-blue-400 border border-blue-200/60 dark:border-blue-500/20',
  ...
  'emerging': {
    label:   'Emerging',
    classes: 'bg-slate-100 text-slate-600 dark:bg-white/5 dark:text-slate-300 border border-slate-200/60 dark:border-white/10',
